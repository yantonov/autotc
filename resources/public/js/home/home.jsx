...
var AgentListItem = React.createClass({
    ...
    render: function() {
        ...
        return (
            ...
            &nbsp;
            <span className="agent__text agent__name"><a href={this.props.agent.webUrl} target="_blank">{this.props.agent.name}</a></span>
            &nbsp;
            <span className="agent__text agent__status">[{agentStatus}]</span>
            </ListGroupItem>
        );
    }
});
...
var AgentList = React.createClass({
    render: function() {
        if (this.props.showLoader)
            return (
                <div>
                <Loader color="#ddd" size="16px" margin="4px"/>
                </div>
            );
        var selectedMap = {};
